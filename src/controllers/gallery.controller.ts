...
export class GalleryController {
  ...
  static async createGalleryItem(req: AuthRequest, res: Response) {
    try {
      ...
      if (!req.files || !Array.isArray(req.files) || req.files.length !== 2) {
        console.log('❌ Invalid files:', {
          hasFiles: !!req.files,
          filesLength: req.files ? (req.files as any[]).length : 0
        });
        return res.status(400).json({
          success: false,
          error: 'Exactly 2 images are required (before and after)',
          received: req.files ? (req.files as any[]).length : 0
        });
      }
  ...
  static async updateGalleryItem(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params;
  ...
  static async deleteGalleryItem(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params;
